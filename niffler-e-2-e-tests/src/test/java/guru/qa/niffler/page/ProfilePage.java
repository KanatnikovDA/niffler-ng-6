...
public class ProfilePage extends BasePage<ProfilePage> {
    ...
    private final SelenideElement avatar = $("#image__input").parent()
                                                             .$("img");
    ...
    public ProfilePage checkArchivedCategoryExists(String category) {
        ...
        bubblesArchived.find(text(category))
                       .shouldBe(visible);
        return this;
    }
    ...
    public ProfilePage checkPhoto(String path) throws IOException {
        ...
        try (InputStream is = new ClassPathResource(path).getInputStream()) {
            photoContent = Base64.getEncoder()
                                 .encode(is.readAllBytes());
        }
    ...
    @Step("Check profile image")
    @Nonnull
    @SneakyThrows
    public ProfilePage checkProfileImage(@Nonnull BufferedImage expectedProfileImage) {
        BufferedImage actualImage = ImageIO.read(Objects.requireNonNull(profileImage.screenshot()));
        assertFalse(new ScreenDiffResult(actualImage, expectedProfileImage));
        return this;
    }
}
